...
def _save_checkpoint(path, state_dict):
    """Write to a temp file then replace, and use legacy pickle format.

    Avoids zip-writer failures (e.g. ``unexpected pos`` / ``iostream stream error``)
    seen on some Windows setups with sync/AV software.
    """
...
def train_model(model, epochs, opt, loss, batch_size):
    ...
    for epoch in range(epochs):
        ...
        for i, batch in progress:
            ...
            # derive IoU values (only where GT and pred both contain smoke)
        ...
        writer.add_scalar(
            "training iou",
            float(np.average(train_ious)) if train_ious else 0.0,
            epoch,
        )
        ...
        writer.add_scalar(
            "val iou",
            float(np.average(val_ious)) if val_ious else 0.0,
            epoch,
        )
        ...
        # save model checkpoint (absolute path; atomic + non-zip format)
        if epoch % 1 == 0:
